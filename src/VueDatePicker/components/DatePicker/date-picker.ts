...
export const useDatePicker = (
    ...
    const tempRangeSelecting = ref<Date[]>([]);
    ...
    const handleDateSelection = (day: ICalendarDay) => {
        if (tempRangeSelecting.value[0]) {
            emit('range-hovering',
                [
                    tempRangeSelecting.value[0],
                    day,
                    !isDateBefore(getDate(day.value), getDate(tempRangeSelecting.value[0]))
                ]);
        }
    };
    ...
    const handleRangeDatesSelect = (day: ICalendarDay, isNext: boolean) => {
        ...
        if (!tempRange.value[0]) {
            ...
            tempRangeSelecting.value[0] = tempRange.value[0];
